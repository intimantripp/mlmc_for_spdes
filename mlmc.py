...
#         for l in range(L + 1):
#             if dNl[l] > 0:
#                 sums, _ = mlmc_l(l, dNl[l])  # returns [sum(Y), sum(Y^2)]
#                 Nl[l] += dNl[l]
#                 suml[0, l] += sums[0]
#                 suml[1, l] += sums[1]
...
#         # convergence test: if no meaningful sample updates
#         if np.sum(dNl > 0.01 * Nl) == 0:
#             idx_range = np.array([-2, -1, 0])
#             rems = ml[L + idx_range] * 2 ** (alpha * idx_range)
#             rem = np.max(rems) / (2**alpha - 1)

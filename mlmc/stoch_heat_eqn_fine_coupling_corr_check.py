...
def _append_samples(level: int, Pf_block: np.ndarray, Pc_block: np.ndarray, log_dir: str):
    ...
    fp = os.path.join(log_dir, f"level_cc_{level}.csv")
        ...
        if l == 0:
            ...
            for _ in range(timesteps_c):
                ...
                for _ in range(4):
                    ...
                    dWf = (half_cell_noises[0::2, :] + half_cell_noises[1::2, :]) / hf
                    dWc += (half_cell_noises[1:-4:4, :] + half_cell_noises[2:-3:4, :] +
                            half_cell_noises[3:-2:4, :] + half_cell_noises[4:-1:4, :]) / hf

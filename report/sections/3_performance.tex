...
implementations achieve achieve markedly lower costs 
...
Instead, all three methods deliver similar performance.

\section{Parallelisation of MLMC Implementations}

A final consideration in assessing the practical efficiency of MLMC 
concerns parallelism. Monte Carlo methods are naturaly 
suited to parallel computing. Individual samples are 
independent, and can therefore be distributed 
across processors with reduced communication overhead. MLMC 
inhertis this parallel structure, with additional 
opportunities for parallelisation both across samples 
and across levels. 

In high-performance computing environments, more 
sophisticated scheduling strategies can be adopted. 
MLMC offers parallelisation across levels, samples, and 
within sample PDE solvers \cite{drzisga2017scheduling}.
In this dissertation, however, we aim to demonstrate 
...
sufficient to illustrate the substantial speed-ups achievable
in practice. 

We report below the measured wall-clock timings for our 
OpenMP-accelerated implementations of all three coupling 
mechanisms, compared with serial counterparts. 
All runs were again completed on Intel Gold 6538Y+ processors, 
and we report results using 8 threads, which was found to 
offer substantial performance improvement.

...
$\Delta x_L = 2^{-(L+1)}$. A higher $L$ means a finer mesh and a more accurate - but also more 
...
$\mathbb{E}[P_L]$. Since the corrections at each level are independent, its variance is the sum 
...
variance of either term individually. As the level $\ell$ increases, $P_{\ell-1}$ converges to 
$P_{\ell}$, and so we expect that $V_\ell \to 0$.
...
minimises the total cost $C_{\textrm{MLMC}} = \sum N_\ell C_\ell$ is solvable with 
...
step size $\Delta x_L \propto \varepsilon^{1/\alpha}$. Independently, controlling the 
statistical error to $O(\varepsilon^2)$ requires 
$N \propto \varepsilon^{-2}$ samples. The total cost is the product of the number of 
samples and the cost per sample, where $C_L \propto \Delta x_L^{-\gamma}$. Combining these 
...
cost is on the coarsest level, where the cost per sample $C_\ell$ is $O(1)$ 

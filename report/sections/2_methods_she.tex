...
We then approximate each term in this equation. Focusing first on 
...
similarly to the deterministic heat equation 
\cite{suli2025nspdes, lord2014introduction}. 
...
The rescaling factor of a $\frac{1}{2}$ is essential to 
...
This motivated our following two proposed strategies.
\newline
...
constructing coarse noise. We anticipated this 
would result in improved correlation between samples.
\newline
...
uniform grid with spacing $\Delta x$:

\begin{equation*}
    \phi_j(x) = \max(0, 1 - |x-x_j|/ \Delta x).
\end{equation*}
...
\begin{equation*}
    M_{i,i} = \frac{2}{3}\Delta x, \qquad M_{i, i\pm 1} = \frac{1}{6} \Delta x.
\end{equation*}

$K$ is a tridiagonal matrix with entries $K_{ij} =(\phi'_i, \phi'_j)$. This gives

\begin{equation*}
    K_{i,i} = \frac{2}{\Delta x}, \qquad K_{i, i\pm 1} = - \frac{1}{\Delta x}.
\end{equation*}
...
The vector of normal increments has the following expectations:

\begin{align*}
    \mathbb{E}[\mathrm{d}W_i \mathrm{d}W_j] = \mathbb{E}[(\mathrm{d}W, \phi_i) 
    (\mathrm{d}W, \phi_j)] = (\phi_i, \phi_j)\mathrm{d}t\\
    \mathbb{E}[\mathrm{d}W_i^2] = \frac{2}{3}\Delta x \mathrm{d}t, \qquad 
    \mathbb{E}[\mathrm{d}W_i \mathrm{d}W_{i\pm 1}] = \frac{1}{6} \Delta x \mathrm{d}t
\end{align*}


Matrix $M$ is then replaced by the diagonal matrix $\Delta x I$ (mass lumping). 

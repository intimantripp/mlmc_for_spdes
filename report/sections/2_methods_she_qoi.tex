...
$\nu_n^j:=\sqrt{\Delta t\,\Delta x}\,\boldsymbol{\phi}_n^\top \mathbf W^j$.
Then, depending on the coupling scheme:
\begin{equation*}
\mathbb{V}[\nu_n^j]
= \Delta t\,\Delta x\,\boldsymbol{\phi}_n^\top \mathbf Q_N \boldsymbol{\phi}_n
=
\begin{cases}
...
\mathbb{V}[\nu_n^j]=
\begin{cases}
\Delta t, & \text{\emph{NN, CC}},\\[0.4ex]
\Delta t\!\left(1-\dfrac{\theta_n^2}{6}+O(\theta_n^4)\right),\quad
\theta_n=\dfrac{\pi n}{N+1}, & \text{\emph{FE}}.
\end{cases}
\]
\end{lemma}

\begin{proof}
The identity $\mathbb{V}[\nu_n^j]=\Delta t\,\Delta x\,\boldsymbol{\phi}_n^\top \mathbf Q_N \boldsymbol{\phi}_n$
is immediate. For NN and CC, $\mathbf Q_N=I_N$ so $\Delta x\,\boldsymbol{\phi}_n^\top \mathbf Q_N\boldsymbol{\phi}_n=1$.
For FE, $\mathbf Q_N=\tfrac{1}{\Delta x}M$ with $M_{ii}=\tfrac{2}{3}\Delta x$, $M_{i,i\pm1}=\tfrac{1}{6}\Delta x$, whence
$\Delta x\,\boldsymbol{\phi}_n^\top \mathbf Q_N \boldsymbol{\phi}_n
= \boldsymbol{\phi}_n^\top M\boldsymbol{\phi}_n
= \tfrac{2}{3}+\tfrac{1}{3}\cos\theta_n
= 1-\tfrac{\theta_n^2}{6}+O(\theta_n^4)
= 1+\mathcal{O}(\Delta x^2)$, where $\theta_n = \frac{\pi n}{N+1}.$\qedhere
\end{proof}
...
\begin{proposition}[Magnitude of MLMC Variance Decay for the Squared Amplitude 
    ...
    \begin{equation*}
    \boxed{V_\ell:= \mathbb{V}\left[P_\ell-P_{\ell-1}\right]
    =\mathcal{O}\big((\Delta x_\ell)^{\min\{p,4\}}\big), \quad
    \text{so } \beta=\min\{p,4\}.}
    \end{equation*}
    
    In particular:
    \begin{itemize}
        \item $p=0 \Rightarrow V_\ell=\Theta(1)$;
        \item $p=2 \Rightarrow V_\ell=\Theta((\Delta x_\ell)^2)$;
        \item $p\ge4 \Rightarrow V_\ell=\mathcal O((\Delta x_\ell)^4)$.
    \end{itemize}
\end{proposition}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \mathcal{O}\left((\Delta x_\ell)^4\right) \\
          &= O\!\big((\Delta x_\ell)^p\big)
        + O\!\big((\Delta x_\ell)^{p+2}\big)
        + O\!\big((\Delta x_\ell)^4\big)
        \qquad\text{if } 1-\rho^{2}=\mathcal{O}\big((\Delta x_\ell)^p\big)\\
        &= O\!\big((\Delta x_\ell)^{\min\{p,4\}}\big).\\
    \end{align*}
\end{proof}

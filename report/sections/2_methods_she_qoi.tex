...
our MLMC version wishes to converge to, and also 
determine what the weak error and variance decay values, $\alpha$ and 
$\beta$ respectively, that we expect to observe for the squared amplitude QoI.
In the next section,
we validate that our implementation aligns with these results. 
...
\begin{align}
    ...
    \nonumber
    \\
    \mathbb{E}\left[u_n(t)^2\right] &= \frac{1 - e^{-2\lambda_nt}}{2\lambda_n}
...
We now establish theoretical decay
rates for our MLMC estimator. We must analyse two key properties:
...
\begin{proposition}[Weak Error for the Squared Amplitude of a Fourier Mode]
    ...
    governing the $n$-th Fourier mode, under the NN, CC, and FE coupling 
    methods, and with time step $\Delta t$, has a weak error 
...
\begin{equation}\label{eq:stationary_variance}
    ...
    := \mathbb{V}[u_n^{\infty}] = \frac{1}{2\lambda_n}
\end{equation}
...
\begin{align}
  ...
  \begin{bmatrix}
    W_1^j \\ \vdots \\ W_N^j
  \end{bmatrix}
  \sim \mathcal{N}(\mathbf{0},\,\mathbf{Q}_N), \quad
  \{\mathbf{W}^j\}_{j\ge 0}\ \text{independent in $j$},\\[2mm]
  \mathbf{U}^{j+1} &= \mathbf{U}^j + \Delta t\,A\,\mathbf{U}^j
  \;+\; \sqrt{\frac{\Delta t}{\Delta x}}\;\mathbf{W}^j,
  \label{eq:scheme_matrix_vector}\\
  A &= \frac{1}{(\Delta x)^2}
  \begin{bmatrix}
    -2 & 1  &        &        \\
     1 & -2 & 1      &        \\
...
Here $\mathbf{Q}_N$ is a symmetric positive-definite per-step spatial covariance matrix. It 
describes the spatial correlations within a time step induced by the coupling methods. 
In the baseline (non-MLMC) case, the NN method, and the CC method, within 
a single level have
$\mathbf{Q}_N=\mathbf{I}_N$. For the FE coupling mechanism, we have 
covariance between within-level noises, such that 
$\mathbf{Q}_N = \frac{1}{\Delta x} M$, where $M$ is the FE mass matrix
in equation \eqref{eq:fe_sdes_system}.
...
\begin{equation*}
    ...
    = \Delta x \boldsymbol{\phi}_n^T \boldsymbol{\phi}_m
    = \sum_{i=1}^N \Delta x (\boldsymbol{\phi}_n)_i (\boldsymbol{\phi}_m)_i = \delta_{n,m},
\end{equation*}
...
\begin{align}
    \begin{split}
        ...
        (\mathbf{W}^j, \boldsymbol{\phi}_n)_{\Delta x},
    \end{split}\\
    (\mathbf{U}^{j+1}, \boldsymbol{\phi}_n)_{\Delta x} &=
    ...
    (\mathbf{W}^j, \boldsymbol{\phi}_n)_{\Delta x} &= 
    \sqrt{\frac{\Delta t}{\Delta x}} \Delta x \boldsymbol{\phi}_n^T \mathbf{W}^j = 
    \sqrt{\Delta t \Delta x} \boldsymbol{\phi}_n^T \mathbf{W}^j
    \nonumber
    \\
    &:= \nu_n^j 
    \sim \mathcal{N}(0, 
    \Delta t \Delta x \boldsymbol{\phi}_n^T \mathbf{Q}_N \boldsymbol{\phi}_n),
...
used that a linear functional of a centred Gaussian vector is Gaussian.
...
We now present the following lemma regarding the variance of the random 
variable $\nu_n^j$. 

\begin{lemma}[Variance of noise in discrete Fourier mode update under NN/CC/FE]\label{lem:modal-var}
Let $\{\boldsymbol{\phi}_n\}$ be the discrete sine basis, orthonormal in $(\cdot,\cdot)_{\Delta x}$, and
$\mathbf W^j\sim\mathcal N(\mathbf 0,\mathbf Q_N)$. Define
$\nu_n^j:=\sqrt{\Delta t\,\Delta x}\,\boldsymbol{\phi}_n^T \mathbf W^j$.
Then, depending on the coupling scheme:
\begin{equation*}
\mathbb{V}[\nu_n^j]
= \Delta t\,\Delta x\,\boldsymbol{\phi}_n^T \mathbf Q_N \boldsymbol{\phi}_n
=
\begin{cases}
\Delta t, &\text{\emph{NN, CC}},\\
\Delta t \left(1+\mathcal{O}\left((\Delta x)^2\right)\right),\quad
&\text{\emph{FE}}.
\end{cases}
\end{equation*}
\end{lemma}
...
\begin{proof}
The identity $\mathbb{V}[\nu_n^j]=\Delta t\,\Delta x\,\boldsymbol{\phi}_n^T 
\mathbf Q_N \boldsymbol{\phi}_n$
is immediate. For NN and CC, $\mathbf Q_N=I_N$ so 
$\Delta x\,\boldsymbol{\phi}_n^T \mathbf Q_N\boldsymbol{\phi}_n=1$.
For FE, $\mathbf Q_N=\tfrac{1}{\Delta x}M$ with 
$M_{ii}=\tfrac{2}{3}\Delta x$, $M_{i,i\pm1}=\tfrac{1}{6}\Delta x$,
thus $\Delta x\,\boldsymbol{\phi}_n^T \mathbf Q_N \boldsymbol{\phi}_n
= \boldsymbol{\phi}_n^T M\boldsymbol{\phi}_n
= \tfrac{2}{3}+\tfrac{1}{3}\cos\theta_n
= 1-\tfrac{\theta_n^2}{6}+O(\theta_n^4)
= 1+\mathcal{O}\left((\Delta x)^2\right)$, where $\theta_n = \frac{\pi n}{N+1}.$\qedhere
\end{proof}

Taking the variance of equation \eqref{eq:inner_product_var_res} gives
\begin{equation}\label{eq:var_of_relation_general}
  \mathbb{V}[\hat u_n^{j+1}]
  = (1-\Delta t\,\lambda_n^{FD})^2\,\mathbb{V}[\hat u_n^{j}]
    \;+\; \Delta t\,\big(1 + \mathcal{O}\left((\Delta x)^2\right)\big),
\end{equation}
where we have assumed the most complicated case of 
being under FE coupling following Lemma~\ref{lem:modal-var}. The 
following holds if $\mathbb{V}[\nu_n^j] = \Delta t$ also.
...
\begin{align}
\mathbb{V}[\hat u_n^\infty]
&= \frac{\Delta t\,(1 + \mathcal{O}\left((\Delta x)^2\right))}
       {1-(1-\Delta t\,\lambda_n^{FD})^2}
= \frac{1 + \mathcal{O}\left((\Delta x)^2\right)}
       {2\,\lambda_n^{FD} - (\lambda_n^{FD})^2\,\Delta t},
\label{eq:fourier_variance_stationary}
\\
&= \frac{1 + \mathcal{O}\left((\Delta x)^2\right)}
{2\lambda_n^{FD}(1 - \frac{\Delta t \lambda_n^{FD}}{2})}
\nonumber
\\
&= \frac{1 + \mathcal{O}\left((\Delta x)^2\right)}{2\lambda_n^{FD}}(1 + \frac{\Delta t \lambda_n^{FD}}{2}
...
\begin{align*}
    ...
    \frac{1 + \mathcal{O}\left((\Delta x)^2\right)}{2\lambda_n^{FD}}\left(1 + 
    \frac{\Delta t \lambda_n^{FD}}{2} + \dots\right) \\
    &= \frac{1 + \mathcal{O}\left((\Delta x)^2\right)}
    {2\left(\lambda_n + \mathcal{O}\left((\Delta x)^2\right)\right)}
    \left(1 + \frac{\Delta t \lambda_n^{FD}}{2} + \dots\right) \\
    &= \frac{1 + \mathcal{O}\left((\Delta x)^2\right)}{2 
    \lambda_n \left(1 + \mathcal{O}(\Delta x^2)\right)}
    \left(1 + \frac{\Delta t \lambda_n^{FD}}{2} + \dots\right) \\
    &= \frac{1 + \mathcal{O}\left((\Delta x)^2\right)}{2 
    \lambda_n}\left(1 + \mathcal{O}\left((\Delta x)^2\right)\right)\left(1 + \frac{\Delta t \lambda_n^{FD}}{2} + \dots\right) \\
    &= \frac{1}{2 \lambda_n}\left(1 + \mathcal{O}\left((\Delta x)^2\right)\right)
...
\begin{equation*}
    |\mathbb{V}\left[u_n^\infty\right] - \mathbb{V}\left[\hat{u}_n^\infty]\right]| 
    = |\frac{1}{2\lambda_n} - \frac{1}{2 \lambda_n}\left(1 + \mathcal{O}(\Delta x^2)\right)|
    = \mathcal{O}(\Delta x^2)
\end{equation*}
\end{proof}
...
\begin{proposition}[Magnitude of MLMC Variance Decay for the Squared Amplitude 
    ...
    Specifically, if $1 - \rho^2 = \mathcal{O}\left((\Delta x_\ell)^p\right)$ for 
    some $p \geq 0$, then:
    \begin{itemize}
        \item $p=0 \Rightarrow V_\ell=\mathcal{O}(1)$;
        \item $p=2 \Rightarrow V_\ell=\mathcal{O}((\Delta x_\ell)^2)$;
        \item $p=4 \Rightarrow V_\ell=\mathcal{O}((\Delta x_\ell)^4)$.
    \end{itemize}
\end{proposition}

Following Proposition \ref{prop:variance_decay_fourier}, we conclude for the MLMC implementation:

\begin{equation*}
    V_\ell = 
    \begin{cases}
        \mathcal{O}(2^{-4\ell}), \text{ thus } \beta = 4 \quad  \text{if } 1 - \rho^2 = \mathcal{O}\left((\Delta x)^4\right)\\
        \mathcal{O}(2^{-2\ell}), \text{ thus } \beta = 2 \quad \text{if } 1 - \rho^2 = \mathcal{O}\left((\Delta x)^2\right)\\
    \end{cases}
\end{equation*}
...
\begin{proof}
    ...
    \begin{equation}\label{eq:fourier_mode_var}
        \mathbb{V}[\hat{u}_n^\ell] = 
        \frac{1 + \kappa(\Delta x_\ell)^2}{2 \lambda_n^{FD} - \Delta t_\ell (\lambda_n^{FD})^2},
    \end{equation}

    where $\kappa$ is a constant that equals 0 under the CC and NN couplings, 
    and is $\mathcal{O}(1)$ under FE.
    ...
    number. Recalling that $\lambda_n^{FD} = \frac{4}{(\Delta x)^2} 
    ...
    \begin{align*}
        \mathbb{V}[\hat{u}_n^\ell] &= \frac{1 + \kappa(\Delta x_\ell)^2}
        {2 \lambda_n^{FD} - 
        \Delta t_\ell (\lambda_n^{FD})^2} = 
        \frac{1 + \kappa(\Delta x_\ell)^2}
        {2\lambda_n\left(1 - \frac{2a + \mu \lambda_n^2}{2\lambda_n}
        (\Delta x_\ell)^2 + \mathcal{O}\left((\Delta x_\ell)^4\right)\right)} \\
        &= \frac{1}{2 \lambda_n} \left(1 + 
        \left(\kappa + \frac{2a + \mu \lambda_n^2}{2 \lambda_n}\right) (\Delta x_\ell)^2
        + \mathcal{O}\left((\Delta x_\ell)^4\right) \right)
        \qquad \text{(Taylor Series)} \\
        &= \frac{1}{2\lambda_n} + 
        \underbrace{\left(\kappa + \frac{2a + \mu \lambda_n^2}{4\lambda_n^2}\right)}_{=b}(\Delta x_\ell)^2
    ...
    \begin{align*}
        ...
        &= 3\sigma_\ell^4 - \sigma_\ell^4 = 2\sigma_\ell^4
    \end{align*}

    Finally, we obtain that:

    \begin{equation}\label{eq:P_var_estimates}
        \mathbb{V}[P_\ell] = \frac{2\sigma_\ell^4}{M}, \quad 
    ...
    \begin{equation*}
        ...
        \frac{1}{M}\mathrm{Cov}(\hat{u}_\ell^2, \hat{u}_{\ell-1}^2) 
        = \frac{1}{M}\mathbb{E}[(\hat{u}_n^\ell \hat{u}_n^{\ell-1})^2] 
    ...
    \begin{equation}\label{eq:cov_fourier_var}
        ...
        2\rho^2\sigma_\ell^2 \sigma_{\ell-1}^2 - 
    ...
    \begin{equation}\label{eq:nearly_there_fourier_var}
        V_l = \mathbb{V}[P_l] + \mathbb{V}[P_{\ell-1}] - 
        2\mathrm{Cov}(P_l, P_{\ell-1}) = 
        \frac{2}{M}(\sigma_\ell^4 + \sigma_{\ell-1}^4 -
        2\rho^2 \sigma_\ell^2 \sigma_{\ell-1}^2)
    \end{equation}
    ...
    \begin{align*}
        \sigma_\ell^4 &= \left(\sigma^2 + b (\Delta x_\ell)^2 + 
        \mathcal{O}\left((\Delta x_\ell)^4\right)\right)^2 = 
        \sigma^4 + 
        2 \sigma^2 b (\Delta x_\ell)^2 + \mathcal{O}\left((\Delta x_\ell)^4\right)
        \\
        \sigma_{\ell-1}^4 &= \left(\sigma^2 + 4b (\Delta x_\ell)^2 + 
        \mathcal{O}\left((\Delta x_\ell)^4\right)\right)^2 =
        \sigma^4 + 8\sigma^2 b(\Delta x_\ell)^2 + 
        \mathcal{O}\left((\Delta x_\ell)^4\right)
        \\
        \sigma_\ell^2 \sigma_{\ell-1}^2 &= \left(\sigma^2+ b(\Delta x_\ell)^2 + 
    ...
    \begin{align*}
        ...
        \mathcal{O}\left((\Delta x_\ell)^4\right), \\
    \end{align*}
    from which our proposition follows.
\end{proof}
...
To derive the expected energy, $\mathbb{E}[P_{\text{energy}}]$, 
